...
// ---------- ROUTE (keep at /api/gamify to match frontend) ----------
router.post("/api/gamify", async (req, res) => {
  try {
    const {
      text = "",
      language = "en",
    ...
    const askCount = Math.min(16, Math.max(targetCount + 2, 14)); // ask a bit more → safe filtering
...
- Return ONLY the raw JSON array (no backticks or prose).`
    ...
    // Post-parse global guard & grounding
