// ~/medplat/frontend/src/components/Level2CaseLogicPlus.jsx
import React, { useMemo, useState } from "react";
import useLevel2CaseEngine from "./useLevel2CaseEngine";

export default function Level2CaseLogicPlus({
  text = "",
  gamify = true,
  caseId = "unknown_case",
  model = "gpt-4o-mini",
  ...
  const filteredIndexes = useMemo(() => {
    return mcqs
      .map((_, i) => ({ i, s: selections[i]?.score ?? 0 }))
      .filter(({ s }) =>
        filter === "all" ? true :
        filter === "partial" ? (s > 0 && s < 3) :
        s === 0
      )
      .map(({ i }) => i);
  }, [mcqs, selections, filter]);
    ...
    return (
      <div className="p-4 space-y-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <button className={`px-2 py-1 border rounded ${filter==="all"?"bg-gray-200":""}`} onClick={() => setFilter("all")}>All</button>
          <button className={`px-2 py-1 border rounded ${filter==="partial"?"bg-gray-200":""}`} onClick={() => setFilter("partial")}>Partial</button>
          <button className={`px-2 py-1 border rounded ${filter==="wrong"?"bg-gray-200":""}`} onClick={() => setFilter("wrong")}>Wrong</button>
          <button
            ...
            return (
              <div key={i} id={`q${i}`} className={`p-3 border rounded ${tier}`}>
                ...
                <div className="mt-2 text-sm">
                  ...
                  {chosen?.explanation && (
                    <div className="mt-1">
                      <span className="font-semibold">Explanation for your choice:</span> {chosen.explanation}
                    </div>
                  )}
                  {correctIdx >= 0 && item.choices?.[correctIdx]?.explanation && sel !== correctIdx && (
                    <div className="mt-1">
                      <span className="font-semibold">Why the correct option is best:</span> {item.choices[correctIdx].explanation}
                    </div>
                  )}
                </div>
                ...
                {/* Evidence (optional) */}
                {item.evidence && Array.isArray(item.evidence.metrics) && item.evidence.metrics.length > 0 && (
                  <div className="mt-2 text-xs text-gray-700">
                    <div className="font-semibold mb-1">Evidence:</div>
                    <ul className="list-disc ml-5 space-y-0.5">
                      {item.evidence.metrics.slice(0,3).map((m, k) => <li key={k}>{m}</li>)}
                    </ul>
                    {item.evidence.level && (
                      <div className="mt-1">Quality: {item.evidence.level}</div>
                    )}
                  </div>
                )}
      ...
      <ul className="space-y-2">
        {(q.choices || []).map((c, idx) => {
          ...
          const immediate =
            ...
            answered &&
            idx === chosenIdx &&
            (c.score === 3 ? "bg-green-200" : (c.score >= 1 && c.score < 3) ? "bg-yellow-200" : "bg-red-200");
          ...
          return (
            <li key={idx}>
              <button
                ...
                disabled={answered}
                className={`w-full text-left px-3 py-2 border rounded ${chosen ? "ring-2 ring-blue-400" : ""} ${immediate || ""}`}
              >
                {c.text}
              </button>
            </li>
